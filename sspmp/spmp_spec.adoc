...
// Should probably explicitly mention that SPMP registers are RW in S-mode.
Similar to the PMP architecture, each SPMP entry is described by an 8-bit configuration register bitfield and one XLEN-bit address register. Some SPMP settings additionally use the address register associated with the preceding SPMP entry. Up to 64 SPMP entries are supported.
// May want to add a note that the size restrictions on SPMPs are similar -- either 4, 16, or 64 are supported IIRC.
// Although there's a statement that the implementation must decode all SPMP CSRs, which is a change from PMP
...
// The bolding of WARL is unusual, and stresses the wrong thing IMO.
[[spmpaddr-rv32]]
...
// I don't like "U-mode-only" when it doesn't necessarily prevent S, depending on bits. What about U-accessible?

// Overall, the use of "Enforced" to mean "allowed" seems unusual. May be a language issue?

. Bit 5 and 6 are reserved for future use.

. The A field will be described in <<address-matching>>.
...
In particular, a value of 00 denotes that the entry is disabled.
...
All sseccfg fields defined in this proposal are WARL, and the remaining bits are reserved for future standard use and should always read zero.
...
// So by default we are permissive accessing nothingness?
// Also, it seems the bit name should be changed -- it's more like "Supervisor no-match Denied-or-not"
...
// It may be easier to think of this in a different order: if M, good. If match, depends on the SPMP entry. If no match and S, check bit. If no match and U, denied. Since you can't determine no-match until you've looked for matches, so those have higher priority.
...
// That depends on microarch -- if using TLBs, the SPMP check can be performed only on TLB misses once, and cached.
+
. Sufficient protection is provided by paged virtual memory.
// But VM does not provide the full array of features w.r.t. sharing as SPMP, does it?
...
// That would be tricky if we change to allow SPMP and VM at the same time, it would be hard to detect what caused the fail without SW querying the SPMP registers.
...
// "can still be" or "must be" handled by M-mode?
// (can PMP be delegated to S?)
...
If multiple `sireg` CSRs are assigned to each `siselect`, a jump table or additional calculations would be needed to determine which `sireg` to address.
...
// This is now part of priv spec, so ref should be updated.
